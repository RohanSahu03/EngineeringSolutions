...
function PurchaseItem() {
  ...
  const savePurchaseItemNo = async () => {
    try {
      const res = await axios.post(
        "https://engineeringsolutions.net.in/api/purchaseitem/createPurchaseItemNo",
  ...
  const deleteItem = async (id) => {
    try {
      const res = await axios.delete(
        `https://engineeringsolutions.net.in/api/purchaseitem/deleteproduct/${id}`
      );
      if (res.status === 200) {
        getAllItems();
  ...
  const getLastPurchaseItemNo = async () => {
    try {
      const res = await axios.get(
        `https://engineeringsolutions.net.in/api/purchaseitem/getLastPurchaseItemNo`
      );
      if (res.status === 201) {
        setIndentNumber(res.data.purchaseitemno);
        increase();
      }
      if (res.status === 500) {
                    ...
                    return (
                      <tr key={item._id}>
                        ...
                        <td className="text-center">
                          ...
                          <FaEye
                            onClick={() =>
                              navigate("/dashboard/items/viewPurchaseItem", {
                                state: item,
                              })
                            }
                            style={{ marginLeft: "10px" }}
