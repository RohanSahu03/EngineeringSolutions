...
function ServiceItem() {
  ...
  const saveItem = async (e) => {
    ...
    // }
    if (!formData.saccode) {
      return toast("Please Enter SAC Code");
    }
    ...
    try {
      const config = {
        url: "/createproduct",
        method: "post",
        baseURL: "https://engineeringsolutions.net.in/api/serviceitem",
  ...
  const getAllItems = async () => {
    try {
      const res = await axios.get(
        "https://engineeringsolutions.net.in/api/serviceitem/getproduct"
      );
      if (res.status === 201) {
  ...
  const deleteItem = async (id) => {
    try {
      const res = await axios.delete(
        `https://engineeringsolutions.net.in/api/serviceitem/deleteproduct/${id}`
      );
      if (res.status === 200) {
        getAllItems();
  ...
  const sortByName = () => {
    setFillteredArray([
      ...product.sort((a, b) => a.itemname.localeCompare(b.itemname)),
    ]);
  };

  const sortByItemCode = () => {
    setFillteredArray([
      ...product.sort((a, b) => a.itemcode.localeCompare(b.itemcode)),
    ]);
  };
  //   const sortByRate = () => {
  //     setFillteredArray([
  ...
  const savePurchaseItemNo = async () => {
    try {
      const res = await axios.post(
        "https://engineeringsolutions.net.in/api/serviceitem/createServiceItemNo",
        { serviceitemno: purchaseitemno }
      );
      if (res.status === 201) {
  ...
  const getLastPurchaseItemNo = async () => {
    try {
      const res = await axios.get(
        `https://engineeringsolutions.net.in/api/serviceitem/getLastServiceItemNo`
      );
      if (res.status === 201) {
        setIndentNumber(res.data.serviceitemno);
        increase();
      }
      if (res.status === 500) {
          ...
          <div className="buttons">
            ...
            <Dropdown className="mx-3">
              ...
              <Dropdown.Menu>
                ...
                <Dropdown.Item onClick={sortByItemCode}>
                  Item Code
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
                    ...
                    return (
                      <tr key={item._id}>
                        ...
                        <td className="text-center">
                          ...
                          <FaEye
                            onClick={() =>
                              navigate("/dashboard/items/viewServiceItem", {
                                state: item,
                              })
                            }
                            style={{ marginLeft: "10px" }}
              ...
              <div className="col-md-6  d-flex">
                ...
                <div className="col-md-8">
                  <input
                    ...
                    value={saccode}
                    placeholder={selecteItem?.saccode}
                  />
                </div>
              </div>
            </div>
