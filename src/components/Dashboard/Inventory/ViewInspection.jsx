    ...
    if (recdata == "Approved") {
      try {
        const res = await axios.put(
          `https://engineeringsolutions.net.in/api/inventory/updateInspection/${receivedData?._id}`,
          { status: "Checked" }
        );
        if (res.status === 201) {
  ...
  const getInspectionById = async () => {
    try {
      const res = await axios.get(
        `https://engineeringsolutions.net.in/api/inventory/getinspectionbyid/${receivedData?._id}`
      );
      if (res.status === 201) {
