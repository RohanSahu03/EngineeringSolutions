...
function ViewGin() {
    ...
    if (recdata == "Approved") {
      try {
        const res = await axios.put(
          `https://engineeringsolutions.net.in/api/inventory/updategin/${receivedData?._id}`,
          { status: "Checked" }
        );
        if (res.status === 201) {
  ...
  const getGinById = async () => {
    try {
      const res = await axios.get(
        `https://engineeringsolutions.net.in/api/inventory/getginbyid/${receivedData?._id}`
      );
      if (res.status === 201) {
      ...
      <section className="mt-3" id="pdf-content">
        <div className="container border p-5">
          ...
          <div class="row">
            ...
            <div className="col-md-3 text-start">{quotedata?.ginno}</div>
