    ...
    if (recdata == "Approved") {
      try {
        const res = await axios.put(
          `https://engineeringsolutions.net.in/api/accounts/updatecashinvoice/${receivedData?._id}`,
          { status: "Approved" }
        );
        if (res.status === 201) {
  ...
  const getInspectionById = async () => {
    try {
      const res = await axios.get(
        `https://engineeringsolutions.net.in/api/accounts/getcashinvoicebyid/${receivedData?._id}`
      );
      if (res.status === 201) {
      ...
      <section className="mt-3" id="pdf-content">
        <div className="container border p-5">
          ...
          <div class="row">
            ...
            <div className="col-md-3 text-start">
              {formatDate(quotedata?.invoicedate)}
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div className="col-md-3 text-start">{quotedata?.billno}</div>
            ...
            <div className="col-md-3 text-start">{formatDate(quotedata?.billdate)}</div>
          </div>
          ...
          <div class="row">
            ...
            <div className="col-md-3 text-start">{quotedata?.supplier}</div>
...
export default CashInvoiceSummary;
